...
function thb_add_lazy_class( $html = '', $new_class ) {
	if ('lazyload' === ot_get_option('thb_preload_type', 'preloader')) {
		...
		if ( preg_match( $pattern, $html, $matches ) ) {
			...
			if ( ! in_array( $new_class, $predefined_classes, true ) && ! in_array( 'rev-slidebg', $predefined_classes, true ) ) {
...
function thb_lazy_images_filter( $content ) {
	if ('lazyload' === ot_get_option('thb_preload_type', 'preloader')) {
		...
		foreach ( $matches[0] as $img_html ) {
			...
			if ( ! ( preg_match( $skip_images_regex, $img_html ) ) && ! ( preg_match( $skip_images_regex2, $img_html ) ) ) {
